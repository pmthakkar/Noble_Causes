...
<body class="hold-transition sidebar-mini">
    ...
    if (isset ($_GET["approve"])) {
        ...
        $sql = "UPDATE `applied_book` SET `a_status` = 'approved' WHERE `applied_book`.`book_id` = $book_id;";
        $result = mysqli_query($conn, $sql);
        $approve = true;
    }
    ...
    if (isset ($_GET["reject"])) {
        ...
        try {
            ...
            $mail->isHTML(true);
            $mail->Subject = 'Book Application in Noble Causes';
            $mail->Body = 'Your Application for ' . $b_title . ' book is rejected by Admin';
            $mail->send();
                                ...
                                <div class="card-body">
                                    ...
                                    <table id="example1" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            <?php
                                            $book_id = 0;
                                            $sql = "SELECT * FROM applied_book LEFT JOIN donate_book ON applied_book.book_id = donate_book.book_id LEFT JOIN users ON applied_book.applicant_id = users.user_id where applied_book.a_status='applied'";
                                            $result = mysqli_query($conn, $sql);
                                            ...
                                            <button class='reject center btn btn-sm btn-danger' id='d" . $row['book_id'] . "' data-email='" . $row['email'] . "' data-title='" . $row['title'] . "'>Reject</button>
                                            </td>
                                          </tr>";
                                            }
                                            ?>
        ...
        <script>
            ...
            Array.from(rejects).forEach((element) => {
                element.addEventListener("click", (e) => {
                    ...
                    u_email = e.target.getAttribute('data-email');
                    b_title = e.target.getAttribute('data-title');


                    if (confirm("Are you sure you want to reject to this user?")) {
                        console.log("yes");
                        rejectApplication(book_id, u_email, b_title);
            ...
            function rejectApplication(id, email, title) {
                var xhr = new XMLHttpRequest();
                xhr.open("GET", "manage_book_applications?reject=" + id + "&email=" + email + "&title=" + title, true);
            ...
            Array.from(approves).forEach((element) => {
                element.addEventListener("click", (e) => {
                    ...
                    if (confirm("Are you sure you want to approve this donation?")) {
                        console.log("yes");
                        window.location = `manage_book_applications?approve=${book_id}`;
